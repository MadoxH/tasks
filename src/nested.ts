...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    let nonEmpty: Question[] = questions.map(
...
export function getNames(questions: Question[]): string[] {
    const namesOnly: string[] = questions.map((a) => a.name);
    return namesOnly;
}
...
export function sumPoints(questions: Question[]): number {
    const pointsOnly: number[] = questions.map((n) => n.points);
    const sumOfPoints: number = pointsOnly.reduce(
...
export function sumPublishedPoints(questions: Question[]): number {
    const pub: Question[] = getPublishedQuestions(questions);
    const sumPub: number = sumPoints(pub);
    return sumPub;
}
...
export function makeAnswers(questions: Question[]): Answer[] {
    const newAnswers: Answer[] = questions.map((questions) => ({
...
export function publishAll(questions: Question[]): Question[] {
    const published: Question[] = questions.map((questions) => ({
        ...questions,
        published: true
    }));
    return published;
...
export function sameType(questions: Question[]): boolean {
    const same = questions.filter((a) => a.type == "multiple_choice_question");
    let value = false;
    same.length === 0 || same.length === questions.length
        ? (value = true)
        : (value = false);
    return value;
}
...
export function changeQuestionTypeById(
    ...
    if (index >= 0) {
        changedType[index].type = newQuestionType;
        if (newQuestionType !== "multiple_choice_question") {
            changedType[index].options = [];
        }
    }
    return changedType;
}
